...
def check_consensus(conversation_history):
    moderator_prompt = """You are a strict debate moderator. 
    Read the conversation below and decide if the agents have had 
    AT LEAST 2 rounds of back-and-forth discussion AND have reached 
    genuine consensus with no major disagreements remaining.
    Only reply YES if both conditions are met, otherwise reply NO.
...
def debate(question, models, personas, max_rounds=5, min_rounds=2):
    print(f"\nQuestion: {question}\n")
    print("=" * 50)
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if round_num >= min_rounds and check_consensus(conversation_history):
